...
(function()
{
    ...
    jQuery.fn.tableChart = function()
    {
        $(this).each(function()
        {
            ...
            {
                // console.log($table.find('tbody > tr').text());
                var labels = [], dataset = {label: $table.find('thead .chart-label').text(), color: nextAccentColor().toCssStr(), data: []},dataset2 = {label: $table.find('thead .chart-label').text(), color: nextAccentColor().toCssStr(), data: []};
                // console.log(labels);
                var $rows = $table.find('tbody > tr').each(function(idx)
                {
                    var $row = $(this);
                    ...
                    dataset2.data.push(parseInt($row.find('.chart-manday').text()));
                });
                var data = {labels: labels, datasets: [dataset,dataset2]};
