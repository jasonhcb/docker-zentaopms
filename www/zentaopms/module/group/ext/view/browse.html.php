...
<table align='center' class='table table-condensed table-hover table-striped  tablesorter table-fixed' id='groupList'>
  ...
  <tbody>
      ...
      <?php $susers = implode(' ', $superUsers);?>
      <tr class='text-center'>
          <td class='strong'>0</td>
          <td class='text-left'><?php echo $lang->group->superUserName;?></td>
          <td class='text-left'><?php echo $lang->group->superUserDesc;?></td>
          <td class='text-left' title='<?php echo $susers;?>'><?php echo $susers;?></td>
          <td class='text-center'><?php common::printIcon('group', 'managemember', "groupID=0", '', 'list', 'group', '', 'iframe', 'yes');?></td>
      </tr>
      <?php endif;?>
  <?php foreach($groups as $group): ?>
    <?php
    foreach ($groupUsers[$group->id] as $k=>$v) {$groupUsers[$group->id][$k] =  $k.':'.$v;}
    ?>
